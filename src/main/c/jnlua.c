...
static jmethodID invoke_id0 = 0;
static jmethodID invoke_id1 = 0;
...
static const char *bytes2string(lua_State *L, jbyteArray bytes, int len, int pop)
{
	if (!bytes)
		return NULL;
	jbyte *str = (*thread_env)->GetByteArrayElements(thread_env, bytes, JNI_FALSE);
	len = len > -1 ? len : (*thread_env)->GetArrayLength(thread_env, bytes);
	lua_pushlstring(L, (char *)str, len);
	const char *name = lua_tostring(L, -1);
	if (pop & 1)
		lua_pop(L, 1);
	if (!(pop & 2))
	{
		(*thread_env)->ReleaseByteArrayElements(thread_env, bytes, str, JNI_ABORT);
...
static int pushmetafunction_protected(lua_State *L)
{
	...
	const char *className = bytes2string(L, meta_class ? meta_class : (*thread_env)->CallStaticObjectMethod(thread_env, luastate_class, classname_id, meta_obj), -1, 1);
	...
	(*thread_env)->DeleteLocalRef(thread_env, meta_obj);
	(*thread_env)->DeleteLocalRef(thread_env, meta_class);
	(*thread_env)->DeleteLocalRef(thread_env, meta_method);
...
void jcall_setglobal(JNIEnv *env, jobject obj, jlong lua, jstring name)
{
	JNLUA_ENV_L;
	setglobal_name = NULL;
...
void jcall_getfield(JNIEnv *env, jobject obj, jlong lua, jint index, jstring k)
{
	JNLUA_ENV_L;
	getfield_k = NULL;
...
void jcall_gettable(JNIEnv *env, jobject obj, jlong lua, jint index)
{
	JNLUA_ENV_L;
...
/* lua_rawget() */
void jcall_rawget(JNIEnv *env, jobject obj, jlong lua, jint index)
{
	JNLUA_ENV_L;
...
/* lua_rawgeti() */
void jcall_rawgeti(JNIEnv *env, jobject obj, jlong lua, jint index, jint n)
{
	JNLUA_ENV_L;
	if (checkstack(L, JNLUA_MINSTACK) && checktype(L, index, LUA_TTABLE))
	{
...
static jbyte *build_args(lua_State *L, int start, int stop, jobjectArray args, jbyteArray types)
{
...
static void push_args(JNIEnv *env, jobject obj, jlong lua, int start, int stop, jobjectArray args, jbyte *types)
{
	JNLUA_ENV_L;
	jobject result;
	int size = 0;
	for (int i = start; i <= stop; i++)
	{
		...
		{
			types[i] -= 16;
			int size = (*thread_env)->GetArrayLength(thread_env, (jobjectArray)o);
			jbyte *t = malloc(size);
			lua_newtable(L);
			for (int j = 0; j < size; j++)
			{
				t[j] = types[i] % 16;
				push_args(env, obj, lua, j, j, (jobjectArray)o, t);
				lua_rawseti(L, -2, j + 1);
			}
			free(t);
		}
		else
		{
			...
			case LUA_TBOOLEAN:;
				...
				break;
			case LUA_TSTRING:
				bytes2string(L, (jbyteArray)o, -1, 0);
				break;
			case LUA_TNUMBER:;
...
void jcall_table_pair_init(JNIEnv *env, jobject obj, jlong lua, jobjectArray keys, jbyteArray types)
{
	JNLUA_ENV_L;
...
static int check_table(lua_State *L, int index, int is_ref)
{
	if (is_ref)
	{
	...
	if (!lua_istable(L, index))
	{
		if (is_ref)
			lua_pop(L, 1);
		return check(0, illegalargumentexception_class, "illegal type.");
	}
	return index;
}
...
{
	JNLUA_ENV_L;
	if (!(index = check_table(L, index, options & 2)))
		return;
	(*env)->PushLocalFrame(thread_env, 8);
	jbyte *types = (*thread_env)->GetByteArrayElements(thread_env, table_pair_types, 0);
	push_args(env, obj, lua, 0, 0, table_pair_values, types);
	int count = 1;
	if (options & 32)
	{
		if (!lua_next(L, index))
	...
	jbyte *b = build_args(L, -1 * count, -1, table_pair_values, table_pair_types);
	types[0] = b[0];
	lua_pop(L, count);
	if (options & 1)
		lua_remove(L, index);
	(*thread_env)->ReleaseByteArrayElements(thread_env, table_pair_types, types, JNI_COMMIT);
	free(b);
	(*env)->PopLocalFrame(thread_env, NULL);
	JNLUA_DETACH_L;
}
...
static void jcall_table_pair_push(JNIEnv *env, jobject obj, jlong lua, jint index, jint options)
{
	...
	if (!(index = check_table(L, index, options & 2)))
		return;
	(*env)->PushLocalFrame(thread_env, 32);
	...
	for (int i = 0; i <= 1; i++)
	{
		push_args(env, obj, lua, i, i, table_pair_values, types);
		if (i == 0)
		{
			if ((options & 4) > 0)
			{
				...
				if (!is_num)
				{
					...
					check(0, illegalargumentexception_class, "lua_table_pair_push: Cannot use table.insert mode to append an non-integer key.");
					return;
				}
			...
			if ((options & 8) > 0)
			{
				lua_pushvalue(L, -1);
				lua_gettable(L, index);
				b = build_args(L, -1, -1, table_pair_values, table_pair_types);
				lua_pop(L, 1);
				types[0] = b[0];
			}
			...
			if ((options & 4) > 0)
			{
				...
				if (len > size - 1)
				{
					if (types[1] == LUA_TNIL)
					{
						jcall_tablemove(env, obj, lua, index, size + 1, size, 1);
						size = len;
					}
					else
					{
						jcall_tablemove(env, obj, lua, index, size, size + 1, 1);
					}
				}
			}
		}
	...
	if ((options & 4) > 0)
		...
		lua_rawset(L, index);
	(*thread_env)->ReleaseByteArrayElements(thread_env, table_pair_types, types, (options & 8) > 0 ? JNI_COMMIT : JNI_ABORT);
	if (options & 1)
		lua_remove(L, index);
	if ((options & 8) > 0)
		free(b);
...
static JNINativeMethod luastate_native_map[] = {
	...
	{"lua_getfield", "(JILjava/lang/String;)V", (void *)jcall_getfield},
	{"lua_getglobal", "(JLjava/lang/String;)V", (void *)jcall_getglobal},
	...
	{"lua_gettable", "(JI)V", (void *)jcall_gettable},
	...
	{"lua_rawget", "(JI)V", (void *)jcall_rawget},
	{"lua_rawgeti", "(JII)V", (void *)jcall_rawgeti},
	...
	{"lua_setglobal", "(JLjava/lang/String;)V", (void *)jcall_setglobal},
	...
	{"lua_table_pair_init", "(J[Ljava/lang/Object;[B)V", (void *)jcall_table_pair_init},
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved)
{
	...
	if (!(javafunction_interface = referenceclass(env, "com/naef/jnlua/JavaFunction")) //
		|| !(invoke_id0 = (*env)->GetMethodID(env, javafunction_interface, "JNI_call", "(Lcom/naef/jnlua/LuaState;J)I")) || !(invoke_id1 = (*env)->GetMethodID(env, javafunction_interface, "JNI_call", "(Lcom/naef/jnlua/LuaState;J[B[Ljava/lang/Object;)I")))
	{
		return JNLUA_JNIVERSION;
	}
	...
	if (!(double_class = referenceclass(env, "java/lang/Double"))												   //
		...
		|| !(number_class = referenceclass(env, "java/lang/Number")) || !(double_value_id = (*env)->GetMethodID(env, double_class, "doubleValue", "()D")))
	{
		return JNLUA_JNIVERSION;
	}
...
static int calljavafunction(lua_State *L)
{
	...
	else
	{
		...
		const jbyteArray types = (*thread_env)->NewByteArray(thread_env, n);
		jbyte *b = build_args(L, 1, n, args, types);
